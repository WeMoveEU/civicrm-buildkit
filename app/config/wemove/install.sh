...
pushd "${WEB_ROOT}/sites/${DRUPAL_SITE_DIR}" >> /dev/null
  ...
  ## Setup CiviCRM
  echo '{"enable_components":["CiviEvent","CiviContribute","CiviMail","CiviReport","CiviPledge","CiviCampaign","CiviMember","CiviGrant","CiviCase"]}' \
  ...
  ## WeMove settings
  cv -vv scr "$SITE_CONFIG_DIR/install-settings.php"
...
EOPERM
  ...
  ## Install extensions
  for e in $(cat "$SITE_CONFIG_DIR/enabled_extensions.txt")
  do
    drush -y -u "$ADMIN_USER" cvapi extension.install key=$e debug=1
  done
